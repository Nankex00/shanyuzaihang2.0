...
import com.fushuhealth.recovery.common.constant.OldRoleEnum;
...
import com.fushuhealth.recovery.common.util.FileUtil;
import com.fushuhealth.recovery.common.util.StringUtil;
import com.fushuhealth.recovery.common.util.VideoTool;
...
//
    ...
    public ListActionResponse queryActions(ActionQuery actionQuery, LoginVo loginVo) {
        ...
        if (!OldRoleEnum.ROLE_SUPER_ADMIN.getName().equalsIgnoreCase(loginVo.getRoleName())) {
...
//        actions.stream().map(actionListVo -> fillUrl(actionListVo)).collect(Collectors.toList());
    ...
    public void saveAction(SaveActionRequest request, LoginVo vo) {
        ...
        OldFileType fileType = OldFileType.getType(request.getFileInfo().getBucket());
...
//        return actionsDao.selectList(new LambdaQueryWrapper<>()).stream().map(Actions::getId).collect(Collectors.toList());
//    }
